...
def getActiveState(node):
    element = node.mElement
    try:
        if parse_WillNotDraw(element):
            print "Will Not Draw!"
            return False
        if not getVisibleState(node):
            print "Not Visible!"
            return False
        if not getClickableState(node):
            print "Not Clickable!"
            return False
